...
def pre(step):
    ...
    lr = 5e-5
    weight_decay = 0.01
    vocab_path = './pretrain/RoBERTa-zh-base/vocab.txt'

    #train_file = './data/train.tsv'
    predict_file = './data/test-id.tsv'
    config = json.load(open('./pretrain/RoBERTa-zh-base/roberta_config.json'))
    ...
    save_type = 'ckpt'
    print_steps = 10000
    pre_params = './pretrain/RoBERTa-zh-base/params'
    ...
    pred_roberta = palm.backbone.BERT.from_config(config, phase='predict')

    # step 3: register the backbone in reader
    predict_cls_reader.register_with(pred_roberta)
    ...
    trainer.build_predict_forward(pred_roberta, cls_pred_head)
...
if __name__ == '__main__':
    models = glob('./outputs-roberta/ckpt.step30000')
#+str(sys.argv[1]))
    for m in models:
        pre(m)
        res_evaluate(res_dir="./outputs-roberta/predict/predictions.json", eval_phase='tid')
