...
def pre(step):
    ...
    lr = 5e-5
    weight_decay = 0.01
    vocab_path = './pretrain/ERNIE-v1-zh-base-max-len-512/vocab.txt'

    #train_file = './data/train.tsv'
    predict_file = './data/train.tsv'
    config = json.load(open('./pretrain/ERNIE-v1-zh-base-max-len-512/ernie_config.json'))
    ...
    task_name = 'news'
    save_path = './outputs/'
    pred_output = './outputs-train/ernie/'
    save_type = 'ckpt'
    print_steps = 10000
    pre_params = './pretrain/ERNIE-v1-zh-base-max-len-512/params'
...
if __name__ == '__main__':
    models = glob('./outputs2/36000')
#+str(sys.argv[1]))
    for m in models:
        pre(m)
        #res_evaluate(res_dir="./outputst/predict/predictions.json", eval_phase='tid')
