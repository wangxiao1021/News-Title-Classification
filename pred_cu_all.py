...
def pre(step, prefix, predict_file):
    # configs
    max_seqlen = 256
    batch_size = 128
    num_epochs = 10
...
if __name__ == '__main__':
    ...
    prefix = './outputs-bert-cu/'  #!!!!!!!!!!!!!!!!!!
    predict_file = './data/cu-all.tsv' #!!!!!!!!!!!!


    res_evaluate(res_dir=prefix+"predict/predictions.json", data_dir=predict_file)
    models = glob(prefix+'ckpt.step*')
    #+str(sys.argv[1]))
    for m in models:
        pre(m, prefix, predict_file)
        res_evaluate(res_dir=prefix+"predict/predictions.json", eval_phase='cu', data_dir=predict_file)
