...
def pre(step):
    ...
    task_name = 'news'
    save_path = './outputs/'
    pred_output = './outputs-trible/predict-roberta/'
    save_type = 'ckpt'
    print_steps = 10
    pre_params = './pretrain/RoBERTa-v1-zh-base-max-len-512/params'
    ...
    predict_cls_reader = palm.reader.ClassifyReader(vocab_path, max_seqlen, seed=random_seed, phase='predict', lang='cn')
...
if __name__ == '__main__':
    models = glob('./outputs-roberta/ckpt.step*')
    for m in models:
        pre(m)
        res_evaluate(res_dir="./outputs1/predict/predictions.json", eval_phase='dev')
